...
### Networks
There are two separate networks in the setup. The cluster-net network is used for communication between the Cassandra 
...
Unlike REST, which typically uses JSON, gRPC relies on binary protocols (Protocol Buffers). This results in smaller message 
