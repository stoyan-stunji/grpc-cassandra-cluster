             ...
             Closeable ignored = es::shutdown)
        {
            Runnable test = () ->
            {
                ...
                try (JMXConnector jmxc = JMXUtil.getJmxConnector(config))
                {
                    ...
                    //assertEquals(sp.getOperationMode(), StorageService.Mode.JOINING.toString()); // FIXME: CASSANDRA-20795 Operation mode is STARTING instead of JOINING during bootstrap
                }
                catch (Throwable t)
                {
