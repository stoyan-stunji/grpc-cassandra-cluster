...
public class PaxosState implements PaxosOperationLock
{
    ...
    public static final Map<Key, Snapshot> RECENT = Caffeine.newBuilder()
                                                            ...
                                                            .<Key, Snapshot>weigher((k, v) -> Ints.saturatedCast((v.accepted != null ? v.accepted.getPartitionUpdate().unsharedHeapSize() : 0L) + v.committed.getPartitionUpdate().unsharedHeapSize()))
    ...
    public static class Snapshot
    {
        ...
        public Snapshot(@Nonnull Ballot promised, @Nonnull Ballot promisedWrite, @Nullable Accepted accepted, @Nonnull Committed committed)
        {
            ...
            assert accepted == null || accepted.getPartitionUpdate().partitionKey().equals(committed.getPartitionUpdate().partitionKey());
            assert accepted == null || accepted.getPartitionUpdate().metadata().id.equals(committed.getPartitionUpdate().metadata().id);
        ...
        Ballot latestWriteOrLowBound()
        {
            // warn: if proposal has same timestamp as promised, we should prefer accepted
            // since (if different) it reached a quorum of promises; this means providing it as first argument
            Ballot latest = accepted != null && !accepted.getPartitionUpdate().isEmpty() ? accepted.ballot : null;
        ...
        Snapshot removeExpired(long nowInSec)
        {
            ...
            {
                long expireOlderThan = SECONDS.toMicros(nowInSec - committed.getPartitionUpdate().metadata().params.gcGraceSeconds);
            ...
            return new Snapshot(promised, promisedWrite,
                                ...
                                isCommittedExpired
                                    ? Committed.none(committed.getPartitionUpdate().partitionKey(), committed.getPartitionUpdate().metadata())
                                    : committed);
        }
    }
    ...
    {
        return get(commit.getPartitionUpdate().partitionKey(), commit.getPartitionUpdate().metadata());
    }
    ...
    {
        return getUnsafe(commit.getPartitionUpdate().partitionKey(), commit.getPartitionUpdate().metadata());
    }
    ...
    public static void commitDirect(Commit commit)
    {
        applyCommit(commit, null, (apply, ignore) -> {
            try (PaxosState state = tryGetUnsafe(apply.getPartitionUpdate().partitionKey(), apply.getPartitionUpdate().metadata()))
            {
                if (state != null)
    ...
    private static void applyCommit(Commit commit, PaxosState state, BiConsumer<Commit, PaxosState> postCommit)
    {
        ...
        try
        {
            ...
            if (commit.ballot.unixMicros() >= SystemKeyspace.getTruncatedAt(commit.getPartitionUpdate().metadata().id))
            {
        ...
        {
            Keyspace.openAndGetStore(commit.getPartitionUpdate().metadata()).metric.casCommit.addNano(nanoTime() - start);
        }
    }
    ...
    public static PrepareResponse legacyPrepare(Commit toPrepare)
    {
                ...
                while (true)
                {
                    ...
                    if (toPrepare.isAfter(latest))
                    {
                        ...
                        if (currentUpdater.compareAndSet(unsafeState, realBefore, after))
                        {
                            ...
                            DecoratedKey partitionKey = toPrepare.getPartitionUpdate().partitionKey();
                            TableMetadata metadata = toPrepare.getPartitionUpdate().metadata();
        ...
        {
            Keyspace.openAndGetStore(toPrepare.getPartitionUpdate().metadata()).metric.casPrepare.addNano(nanoTime() - start);
        }
    }
        ...
        try (PaxosState unsafeState = getUnsafe(proposal))
        {
            synchronized (unsafeState.key)
            {
                ...
                while (true)
                {
                    ...
                    if (accept)
                    {
                        PartitionUpdate partitionUpdate = proposal.getPartitionUpdate();
