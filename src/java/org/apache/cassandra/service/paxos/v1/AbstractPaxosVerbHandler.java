...
public abstract class AbstractPaxosVerbHandler implements IVerbHandler<Commit>
{
    ...
    public void doVerb(Message<Commit> message)
    {
        ...
        DecoratedKey key = commit.getPartitionUpdate().partitionKey();
        if (isOutOfRangeCommit(commit.getPartitionUpdate().metadata().keyspace, key))
        {
            StorageService.instance.incOutOfRangeOperationCount();
            Keyspace.open(commit.getPartitionUpdate().metadata().keyspace).metric.outOfRangeTokenPaxosRequests.inc();

            // Log at most 1 message per second
                NoSpamLogger.log(logger, NoSpamLogger.Level.WARN, 1, TimeUnit.SECONDS, logMessageTemplate, message.from(), key.getToken(), commit.getPartitionUpdate().metadata().keyspace);
