...
public class ConsensusReadForwardHandler implements IVerbHandler<ConsensusReadForwardRequest>
{
    ...
    public void doVerb(Message<ConsensusReadForwardRequest> message)
    {
        ...
        try
        {
            ...
            {
                MessagingService.instance().respondWithFailure(RequestFailureReason.INCOMPATIBLE_SCHEMA, message);
                logger.error("Asked to perform forwarded consensus read operation, but keyspace " + keyspaceName + " is not tracked");
                return;
            }
            ...
            ConsensusReadForwardResponse response = new ConsensusReadForwardResponse(result, warnings);
        ...
        catch (CassandraException ce)
        {
            ...
            List<String> warnings = ClientWarn.instance.getWarnings();
            ConsensusReadForwardResponse response = new ConsensusReadForwardResponse(ce, warnings);
            MessagingService.instance().respond(response, message);
        }
        catch (Throwable t)
        {
