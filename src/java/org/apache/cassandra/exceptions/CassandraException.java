...
public abstract class CassandraException extends RuntimeException implements TransportException
{
    ...
    public static class Serializer implements IVersionedSerializer<CassandraException>
    {
        ...
        {
            // Serialize new CassandraExceptionCode for proper type identification
            CassandraExceptionCode cassandraExceptionCode = exception.getCassandraExceptionCode();
            out.writeUnsignedVInt32(cassandraExceptionCode.value);

            String message = exception.getMessage() == null ? "" : exception.getMessage();
            out.writeUTF(message);

            // Serialize cause and suppressed exceptions using ExceptionSerializer
            ...
            {
                ExceptionSerializer.remoteExceptionSerializer.serialize(t, out, version);
            }
        ...
        public CassandraException deserialize(DataInputPlus in, int version) throws IOException
        {
            ...
            CassandraExceptionCode classCode = CassandraExceptionCode.fromValue(cassandraExceptionCode);

            String message = in.readUTF();

            // Deserialize cause and suppressed exceptions using ExceptionSerializer
            ...
            {
                suppressed[i] = ExceptionSerializer.remoteExceptionSerializer.deserialize(in, version);
            }

            // Deserialize stack trace using existing ExceptionSerializer logic
            StackTraceElement[] stackTrace = ArraySerializers.deserializeArray(in, version, ExceptionSerializer.stackTraceElementSerializer, size -> new StackTraceElement[size]);

            // Create the appropriate exception instance using the class code
            CassandraException exception = deserializeByClassCode(classCode, message, in, version);

            // Set cause, suppressed, and stack trace
            if (cause != null)
        ...
        public long serializedSize(CassandraException exception, int version)
        {
            ...
            long size = TypeSizes.sizeofUnsignedVInt(exception.getCassandraExceptionCode().value); // class code value
            size += TypeSizes.sizeof(message);

            // Size for cause and suppressed exceptions using ExceptionSerializer
            size += ExceptionSerializer.nullableRemoteExceptionSerializer.serializedSize(exception.getCause(), version);

            Throwable[] suppressed = exception.getSuppressed();
            size += TypeSizes.sizeofUnsignedVInt(suppressed.length); // suppressed count
            for (Throwable t : suppressed)
            {
                size += ExceptionSerializer.remoteExceptionSerializer.serializedSize(t, version);
            }

            // Size for stack trace using existing ExceptionSerializer logic
